package supply

import (
	"github.com/beatoz/beatoz-go/ctrlers/types"
	"github.com/beatoz/beatoz-go/ledger/common"
	btztypes "github.com/beatoz/beatoz-go/types"
	"github.com/beatoz/beatoz-go/types/bytes"
	"github.com/beatoz/beatoz-go/types/xerrors"
	"github.com/holiman/uint256"
)

// distReward distributes applies `mintedReward` to `supplyState`.
// this is called from waitMint that is called from EndBlock.
func (ctrler *SupplyCtrler) distReward(rewards []*mintedReward, height int64, poolAddr btztypes.Address) xerrors.XError {
	if poolAddr != nil && !bytes.Equal(poolAddr, btztypes.ZeroAddress()) {
		ctrler.logger.Info("reward pool is not supported yet.")
		return ctrler.distRewardToState(rewards, height)
	} else {
		return ctrler.distRewardToState(rewards, height)
	}
}

func (ctrler *SupplyCtrler) distRewardToState(rewards []*mintedReward, height int64) xerrors.XError {
	for _, nrwd := range rewards {
		item, xerr := ctrler.supplyState.Get(common.LedgerKeyReward(nrwd.addr), true)
		if xerr != nil && !xerr.Contains(xerrors.ErrNotFoundResult) {
			return xerr
		}

		rwd, _ := item.(*Reward)
		if rwd == nil {
			rwd = NewReward(nrwd.addr)
		}
		_ = rwd.Issue(nrwd.amt, height)

		if xerr := ctrler.supplyState.Set(common.LedgerKeyReward(nrwd.addr), rwd, true); xerr != nil {
			return xerr
		}
	}
	return nil
}

func (ctrler *SupplyCtrler) distRewardToPool() xerrors.XError {
	panic("not supported yet")
}

func (ctrler *SupplyCtrler) readReward(addr btztypes.Address) (*Reward, xerrors.XError) {
	item, xerr := ctrler.supplyState.Get(common.LedgerKeyReward(addr), true)
	if xerr != nil {
		return nil, xerr
	}

	return item.(*Reward), nil
}

// withdrawReward calls `currReward.Withdraw` to refund `amt` from `currReward`.
// this is called from ExecuteTrx.
func (ctrler *SupplyCtrler) withdrawReward(currReward *Reward, amt *uint256.Int, height int64, acctHandler types.IAccountHandler, exec bool) xerrors.XError {
	_ = currReward.Withdraw(amt, height)
	if currReward.CumulatedAmount().IsZero() {
		if xerr := ctrler.supplyState.Del(common.LedgerKeyReward(currReward.Address()), exec); xerr != nil {
			return xerr
		}
	} else {
		if xerr := ctrler.supplyState.Set(common.LedgerKeyReward(currReward.Address()), currReward, exec); xerr != nil {
			return xerr
		}
	}

	if xerr := acctHandler.Reward(currReward.Address(), amt, exec); xerr != nil {
		return xerr
	}
	return nil
}
