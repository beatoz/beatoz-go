...
func (ctrler *GovCtrler) EndBlock(ctx *types.BlockContext) ([]types2.Event, xerrors.XError) {
	...
	for _, k := range frozen {
		evts = append(evts, types2.Event{
			Type: "proposal",
			Attributes: []types2.EventAttribute{
				{Key: []byte("frozen"), Value: []byte(hex.EncodeToString(common.UnwrapKeyPrefix(k))), Index: true},
			},
		})
	}
	for _, k := range removed {
		evts = append(evts, types2.Event{
			Type: "proposal",
			Attributes: []types2.EventAttribute{
				{Key: []byte("removed"), Value: []byte(hex.EncodeToString(common.UnwrapKeyPrefix(k))), Index: true},
			},
		})
	}
	for _, k := range applied {
		evts = append(evts, types2.Event{
			Type: "proposal",
			Attributes: []types2.EventAttribute{
				{Key: []byte("applied"), Value: []byte(hex.EncodeToString(common.UnwrapKeyPrefix(k))), Index: true},
			},
		})
	}
	for _, k := range rejected {
		evts = append(evts, types2.Event{
			Type: "proposal",
			Attributes: []types2.EventAttribute{
				{Key: []byte("rejected"), Value: []byte(hex.EncodeToString(common.UnwrapKeyPrefix(k))), Index: true},
			},
		})
	}
