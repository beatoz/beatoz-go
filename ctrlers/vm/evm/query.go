...
func (ctrler *EVMCtrler) callVM(from, to types.Address, data []byte, height, blockTime int64) (*core.ExecutionResult, xerrors.XError) {
	...
	_gasLimit := uint64(math.MaxUint64 / 2)
	vmmsg := evmMessage(sender, toAddr, 0, _gasLimit, uint256.NewInt(0), uint256.NewInt(0), data, true)
	blockContext := newEVMBlockContext(sender, height, blockTime, _gasLimit)
	gp := new(core.GasPool).AddGas(_gasLimit)
