...
func Test_LoadLedger(t *testing.T) {
	...
	require.NoError(t, ctrler.LoadDelegatees(int(govMock.MaxValidatorCnt()), govMock.MinValidatorPower(), govMock.MinSelfPowerRate()))
...
func Test_Delegating_SelfPowerRatioBoundary(t *testing.T) {
	require.NoError(t, os.RemoveAll(config.RootDir))
	...
	require.NoError(t, xerr)
	require.Equal(t, int32(50), govMock.MinSelfPowerRate())
	require.Equal(t, dgteeBefore.SelfPower, dgteeBefore.SumPower)

	// The external delegation that leaves exactly 50% self-power is allowed.
	_, xerr = doDelegate(ctrler, acctMock.RandWallet(), target.Address(), dgteeBefore.SelfPower, lastHeight+1)
	require.NoError(t, xerr)

	dgteeAtBoundary, xerr := ctrler.readDelegatee(target.Address(), true)
	require.NoError(t, xerr)
	require.True(t, hasEnoughSelfPower(
		dgteeAtBoundary.SelfPower,
		dgteeAtBoundary.SumPower,
		govMock.MinSelfPowerRate(),
	))
	selfPowerAtBoundary := dgteeAtBoundary.SelfPower
	sumPowerAtBoundary := dgteeAtBoundary.SumPower

	// Any further external delegation would lower the self-power ratio below 50%.
	_, xerr = doDelegate(ctrler, acctMock.RandWallet(), target.Address(), govMock.MinDelegatorPower(), lastHeight+1)
	require.Error(t, xerr)
	require.Contains(t, xerr.Error(), "not enough self power")

	dgteeAfterRejected, xerr := ctrler.readDelegatee(target.Address(), true)
	require.NoError(t, xerr)
	require.Equal(t, sumPowerAtBoundary, dgteeAfterRejected.SumPower)
	require.Equal(t, selfPowerAtBoundary, dgteeAfterRejected.SelfPower)
...
func Test_Unbonding_AllSelfPower(t *testing.T) {
	...
	_, vals, _, _ := testRandDelegate(t, 1000, ctrler, valWallets, lastHeight+1)
	...
	for i, valWal := range onceWals {
		...
		dgteeBefore, xerr := ctrler.readDelegatee(valWal.Address(), true)
		require.NoError(t, xerr)
		type refundExpectation struct {
			from  []byte
			power int64
		}
		var expectedRefunds []refundExpectation
		refundHeight := lastHeight + 1 + govMock.LazyUnbondingBlocks()
		for _, from := range dgteeBefore.Delegators {
			vpow, xerr := ctrler.readVPower(from, dgteeBefore.addr, true)
			require.NoError(t, xerr)

			previousRefund := int64(0)
			frozen, xerr := ctrler.readFrozenVPower(refundHeight, from, true)
			if xerr == nil {
				previousRefund = frozen.RefundPower
			} else {
				require.True(t, xerr.Contains(xerrors.ErrNotFoundResult))
			}
			expectedRefunds = append(expectedRefunds, refundExpectation{
				from:  append([]byte(nil), from...),
				power: previousRefund + vpow.SumPower,
			})
		}
		...
		// expected that all vpowers delegated to dgtee are removed and frozen exactly once
		for _, expected := range expectedRefunds {
			vpow, xerr := ctrler.readVPower(expected.from, valWal.Address(), true)
			require.Equal(t, xerrors.ErrNotFoundResult, xerr)
			require.Nil(t, vpow)

			frozen, xerr := ctrler.readFrozenVPower(refundHeight, expected.from, true)
			require.NoError(t, xerr)
			require.Equal(t, expected.power, frozen.RefundPower)
			frozenPower := int64(0)
			for _, pc := range frozen.PowerChunks {
				frozenPower += pc.Power
			}
			require.Equal(t, frozen.RefundPower, frozenPower)
		}
		tombstoned, xerr := ctrler.isTombstoned(valWal.Address(), true)
		require.NoError(t, xerr)
...
func Test_Unbonding_PartialSelfPower_Eligibility(t *testing.T) {
	require.NoError(t, os.RemoveAll(config.RootDir))
	...
	_ = mocks.InitBlockCtxWith(config.ChainIdHex(), lastHeight+1, govMock, acctMock, nil, nil, ctrler)
	...
	promotedUp, cnt := findValUp(promoted.GetPubKey(), bctx.ValUpdates)
	require.Equal(t, 1, cnt)
	...
	_, expectedVotingPower := ctrler.Validators()
	queriedVotingPower, xerr := ctrler.queryVotingPower(
		mocks.LastBlockHeight(),
		func() interface{} { return govMock.MaxValidatorCnt() },
		func() interface{} { return govMock.MinValidatorPower() },
		func() interface{} { return govMock.MinSelfPowerRate() },
	)
	require.NoError(t, xerr)
	require.Equal(t, fmt.Sprintf("\"%d\"", expectedVotingPower), string(queriedVotingPower))
	...
	ctrler2.vpowLimiter.Reset(totalPower2, 100)
...
func initLedger(cfg *beatozcfg.Config) (*VPowerCtrler, []abcitypes.ValidatorUpdate, []*web3.Wallet, xerrors.XError) {
	ctrler, xerr := NewVPowerCtrler(cfg, int(govMock.MaxValidatorCnt()), govMock.MinValidatorPower(), govMock.MinSelfPowerRate(), log.NewNopLogger())
	if xerr != nil {
