...
func selectValidators(delegatees []*DelegateeProto, maxVals int) []*DelegateeProto {
	sort.Sort(orderByPowerDelegateeProtos(delegatees))
	ret := make([]*DelegateeProto, libs.MinInt(len(delegatees), maxVals))
...
func validatorUpdates(lastVals, newVals []*DelegateeProto) []abcitypes.ValidatorUpdate {
	valUpdates := make(abcitypes.ValidatorUpdates, 0, len(lastVals)+len(newVals))

	sort.Sort(orderByPubDelegateeProtos(lastVals))
	sort.Sort(orderByPubDelegateeProtos(newVals))
